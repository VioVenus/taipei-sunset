...
import { lightPhase, untilStr } from "../js/light.js";
...
test("golden hour between golden start and sunset", () => {
  ...
  assert.equal(r.phase.key, "golden");
...
test("afterglow window: sunset (inclusive) to civil end", () => {
  assert.equal(lightPhase(sun, sun.sunsetMs).phase.key, "afterglow"); // 日落瞬間即進入
  const mid = lightPhase(sun, 1140 * M);
  assert.equal(mid.phase.key, "afterglow");
...
test("night at/after civil twilight end", () => {
  ...
  assert.equal(r.phase.key, "night");
  assert.equal(r.untilMs, null);
});

test("untilStr formats minutes and hours", () => {
  assert.equal(untilStr(90 * M, 0), "1 小時 30 分");
  assert.equal(untilStr(12 * M, 0), "12 分鐘");
  assert.equal(untilStr(0, 5 * M), "0 分鐘"); // 不出現負數
});
