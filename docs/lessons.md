...
**全台化的草稿點政策（v1.2.0）**：擴充到全台時，為了不違反本教訓，草稿點位
（`needs_field_verification: true`）只提供「地點座標」（來源記於 `coord_source`），
**不偽造遮蔽幾何**——`horizon_obstruction` 一律留空、`open_azimuth_range` 取寬鬆西向；
UI 明確標示「座標待實地確認」，每日推播頭條只用已驗證點位。實地確認視線方位與
遮蔽仰角後，才移除旗標並補入遮蔽資料。禁止直接以草稿充當已驗證。
